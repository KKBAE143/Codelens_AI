...
${changedFileContents ? `\nChanged file contents:\n${changedFileContents.slice(0, 3000)}` : ""}
...
export const regenerateCourseJob = inngest.createFunction(
  ...
  { event: "codelens/course.regenerate" },
  async ({ event, step }) => {
    ...
    const changeSummary = await step.run("generate-change-summary", async () => {
    ...
    await step.run("store-changes-and-mark-generating", async () => {
      const allFiles = [...changedFiles.added, ...changedFiles.modified, ...changedFiles.removed];
      await db
        .update(courses)
        .set({
    ...
    const extraction = await step.run("extract-repo", async () => {
      try {
        ...
        await updateProgress(
          courseId,
          "extracting",
          `Read ${result.files.length} files from ${result.owner}/${result.repoName}`,
          15
        );
        return result;
      } catch (error) {
    ...
    const changedFileContents = extraction.files
      .filter((f: { path: string }) =>
        changedFiles.added.includes(f.path) || changedFiles.modified.includes(f.path)
      )
      .slice(0, 20)
    ...
    const previousAnalysisSummary = course.analysis
      ? JSON.stringify(course.analysis, null, 2).slice(0, 4000)
      : "N/A";
...
INSTRUCTION: Update the course content to reflect these changes. Pay special attention to added/modified files and adjust modules, explanations, and code examples accordingly. Use the previous analysis to understand what was taught before and what needs to change.`;
    ...
    const curriculum = await step.run("stage2-curriculum", async () => {
      ...
      try {
        ...
        await db
          .update(courses)
          .set({ curriculum: result.data, moduleCount: modules.length, estimatedMinutes, updatedAt: new Date() })
          .where(eq(courses.id, courseId));
        return result.data;
      } catch (error) {
