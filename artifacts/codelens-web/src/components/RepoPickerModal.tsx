...
export function RepoPickerModal({ onSelect, onClose }: RepoPickerModalProps) {
  ...
  const [currentUser, setCurrentUser] = useState<{ login: string; avatar_url: string | null } | null>(null);
  ...
  useEffect(() => {
    fetch("/api/github/orgs", { credentials: "include" })
      ...
      .catch((err) => {
        setOrgsError(err instanceof Error ? err.message : "Failed to load accounts");
      });
  }, []);

  const fetchRepos = useCallback(async (org: string, searchQ: string, pg: number) => {
    ...
    try {
      ...
      const res = await fetch(`/api/github/repos?${params}`, { credentials: "include" });
  ...
  const triggerFetch = useCallback((org: string, searchQ: string, pg: number) => {
    if (searchTimeout.current) clearTimeout(searchTimeout.current);
    if (searchQ.trim()) {
      searchTimeout.current = setTimeout(() => fetchRepos(org, searchQ, pg), 350);
    } else {
          ...
          {loading && repos.length === 0 ? (
            <div style={{ padding: "3rem", textAlign: "center", color: "var(--text-tertiary)" }}>
              <div style={{ fontSize: "1.5rem", marginBottom: "0.5rem" }}>
                <span className="spin-icon" style={{ display: "inline-block", animation: "spin 1s linear infinite" }}>&#9696;</span>
              </div>
              Loading repositories...
              <style>{`@keyframes spin { to { transform: rotate(360deg) } }`}</style>
            </div>
          ) : error ? (
            <div style={{ padding: "2rem 1.5rem", textAlign: "center", color: "var(--error)" }}>
              {error}
            </div>
          ) : repos.length === 0 ? (
            <div style={{ padding: "3rem", textAlign: "center", color: "var(--text-tertiary)" }}>
              No repositories found{search ? ` matching "${search}"` : ""} for {accountLabel}.
            </div>
          ) : (
            <div>
              {repos.map((repo) => (
