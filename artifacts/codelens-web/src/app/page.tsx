...
export default function Home() {
  ...
  const [existingCourse, setExistingCourse] = useState<ExistingCourseInfo | null>(null);
          ...
          setDemoCourses(
            DEMOS.map((d) => {
              const match = data.demos.find(
                (m: { repo: string; shareToken: string }) => `${m.repo}` === d.repo
              );
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      Promise.all([
        fetch(`https://api.github.com/repos/${match[1]}/${match[2].replace(/\.git$/, "")}`, { signal: controller.signal })
          .then((r) => (r.ok ? r.json() : null))
          .catch(() => null),
        fetch(`/api/courses/check-existing?url=${encodeURIComponent(url)}`, { signal: controller.signal })
          .then((r) => (r.ok ? r.json() : null))
          .catch(() => null),
    ...
    <>
      <main style={{ minHeight: "100vh" }}>
        <section className="hero-bg" style={{
          ...
          {isAuthenticated && (
            <div style={{
              ...
              <button
                ...
                style={{
                  flex: 1, padding: "0.5rem 1rem", border: "none", borderRadius: "var(--radius-sm)",
          ...
          {(!isAuthenticated || inputMode === "url") ? (
            <div style={{ maxWidth: 540, margin: "0 auto 1rem", position: "relative" }}>
              <input
                ...
                style={{
                  width: "100%", padding: "0.875rem 1.25rem", paddingRight: "2.5rem",
                  border: `2px solid ${isValidUrl && url ? "var(--teal)" : "var(--border-color)"}`,
                  borderRadius: "var(--radius-md)", fontSize: "0.95rem",
                  fontFamily: "var(--font-mono)", background: "white", outline: "none",
                  transition: "border-color 0.2s",
                }}
                onFocus={(e) => { if (!isValidUrl) (e.target as HTMLInputElement).style.borderColor = "var(--accent)"; }}
                onBlur={(e) => { if (!isValidUrl) (e.target as HTMLInputElement).style.borderColor = "var(--border-color)"; }}
              />
              {isValidUrl && url && (
                <span style={{ position: "absolute", right: 12, top: "50%", transform: "translateY(-50%)", color: "var(--teal)", fontSize: "1.25rem" }}>
                  {"\u2713"}
                </span>
              )}
            </div>
            ...
            <div style={{ maxWidth: 540, margin: "0 auto 1rem" }}>
              {url ? (
                <div style={{
                  ...
                  padding: "0.75rem 1rem", border: "2px solid var(--teal)",
                  ...
                  <span style={{ fontFamily: "var(--font-mono)", fontSize: "0.9rem", fontWeight: 600, flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                    {url.replace("https://github.com/", "")}
                  </span>
                  <button onClick={() => { setUrl(""); setRepoPreview(null); }} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--text-tertiary)", fontSize: "1.1rem", padding: "0.25rem", lineHeight: 1 }}>
                    &times;
                  </button>
                </div>
              ) : (
                <button
                  ...
                  style={{
                    ...
                    justifyContent: "center", gap: "0.5rem", transition: "border-color 0.2s, background 0.2s",
                  }}
          ...
          {isValidUrl && repoPreview && (
            <div style={{ maxWidth: 540, margin: "0 auto 1.5rem" }}>
              <div style={{
                ...
                <img src={repoPreview.avatar} alt="" style={{ width: 36, height: 36, borderRadius: "50%" }} />
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontFamily: "var(--font-mono)", fontSize: "0.85rem", fontWeight: 600 }}>{repoPreview.fullName}</div>
                  {repoPreview.description && (
                    <p style={{ fontSize: "0.75rem", color: "var(--text-secondary)", lineHeight: 1.4, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                      {repoPreview.description}
                    </p>
                  )}
                </div>
                <div style={{ display: "flex", gap: "0.375rem", flexShrink: 0 }}>
                  {repoPreview.language && <span className="badge" style={{ background: "var(--teal-light)", color: "var(--teal)" }}>{repoPreview.language}</span>}
                  <span className="badge" style={{ background: "var(--bg-secondary)", color: "var(--text-secondary)" }}>{repoPreview.stars.toLocaleString()} stars</span>
                </div>
              </div>
            </div>
          )}
          ...
          {existingCourse && (
            <div style={{
              maxWidth: 540, margin: "0 auto 1.5rem", padding: "0.875rem 1rem",
              background: "var(--teal-light)", border: "1px solid var(--teal)",
              borderRadius: "var(--radius-md)", textAlign: "left",
            }}>
              <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.35rem" }}>
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--teal)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>
                <span style={{ fontSize: "0.85rem", fontWeight: 600, color: "var(--teal)" }}>Course already exists!</span>
              </div>
              <p style={{ fontSize: "0.8rem", color: "var(--text-secondary)", marginBottom: "0.5rem" }}>
                {existingCourse.oneLiner || `A course for ${existingCourse.ownerName}/${existingCourse.repoName} is already available.`}
              </p>
              <div style={{ display: "flex", gap: "0.5rem", alignItems: "center" }}>
                <Link href={existingCourse.exploreUrl} className="btn-primary" style={{ textDecoration: "none", fontSize: "0.75rem", padding: "0.4rem 0.75rem" }}>
                  View Course
                </Link>
                <span style={{ fontSize: "0.7rem", color: "var(--text-tertiary)" }}>
                  {existingCourse.moduleCount && `${existingCourse.moduleCount} modules`}
                  {existingCourse.moduleCount && existingCourse.estimatedMinutes && " · "}
                  {existingCourse.estimatedMinutes && `~${existingCourse.estimatedMinutes} min`}
                </span>
              </div>
            </div>
          )}
            ...
            <div style={{ maxWidth: 540, margin: "0 auto 1rem" }}>
              <select
                ...
                style={{
                  ...
                  fontSize: "0.85rem", fontFamily: "var(--font-body)", background: "white",
                  outline: "none", color: selectedOrg ? "var(--text-primary)" : "var(--text-tertiary)",
                }}
              >
              ...
              {demoCourses.map((demo) => {
                ...
                return (
                  <div key={demo.repo} className="card" style={{
                    ...
                    opacity: isAvailable ? 1 : 0.65, cursor: isAvailable ? "pointer" : "default",
                    transition: "opacity 0.2s, transform 0.2s",
                  }}
                  onClick={() => { if (isAvailable) window.location.href = `/share/${demo.shareToken}`; }}
                  >
                    <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", flexWrap: "wrap" }}>
                      <code style={{ fontSize: "0.8rem", background: "var(--bg-secondary)", padding: "0.2rem 0.5rem", borderRadius: "var(--radius-sm)", color: "var(--text-primary)", fontWeight: 500 }}>
                        {demo.repo}
                      </code>
                    </div>
                    <p style={{ fontSize: "0.85rem", color: "var(--text-secondary)", lineHeight: 1.5 }}>{demo.desc}</p>
                    <div style={{ display: "flex", gap: "0.375rem", flexWrap: "wrap" }}>
                      ...
                      <span className="badge" style={{ background: "var(--bg-secondary)", color: "var(--text-secondary)" }}>{demo.time}</span>
                    </div>
                    {isAvailable ? (
                      <Link href={`/share/${demo.shareToken}`} className="btn-secondary" style={{ textAlign: "center", textDecoration: "none", fontSize: "0.8rem", padding: "0.5rem" }} onClick={(e) => e.stopPropagation()}>
                        View Course {"\u2192"}
                      </Link>
                    ) : (
        ...
        <footer style={{ padding: "2rem 1.5rem", textAlign: "center", borderTop: "1px solid var(--border-color)" }}>
          <div style={{ display: "flex", justifyContent: "center", gap: "1.5rem", marginBottom: "0.75rem" }}>
            <Link href="/explore" style={{ color: "var(--text-secondary)", textDecoration: "none", fontSize: "0.8rem" }}>Explore Courses</Link>
            <Link href="/pricing" style={{ color: "var(--text-secondary)", textDecoration: "none", fontSize: "0.8rem" }}>Pricing</Link>
          </div>
      ...
      {showRepoPicker && (
        <RepoPickerModal
          onSelect={(repoUrl) => { setUrl(repoUrl); setShowRepoPicker(false); }}
