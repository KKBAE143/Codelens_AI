...
export default function Dashboard() {
  ...
  const { data: assignmentsData } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch("/api/courses/assigned", { credentials: "include" });
  ...
  const invitationMutation = useMutation({
    mutationFn: async ({ invitationId, action }: { invitationId: string; action: string }) => {
  ...
  const handleWebhookToggle = async (courseId: string, currentlyEnabled: boolean) => {
    const action = currentlyEnabled ? "disable" : "enable";
    if (currentlyEnabled && !confirm("Disable auto-updates for this course? The webhook will be removed from GitHub.")) return;
    try {
      ...
      queryClient.invalidateQueries({ queryKey: ["courses"] });
      showToast(currentlyEnabled ? "Auto-updates disabled" : "Auto-updates enabled", "success");
    } catch (err) {
      showToast(err instanceof Error ? err.message : `Failed to ${action} webhook`, "error");
    }
  };

  const handleRegenerate = async (id: string) => {
    if (!confirm("Regenerate this course? This will use 1 generation credit.")) return;
    try {
    ...
    <main style={{ maxWidth: 900, margin: "0 auto", padding: "2.5rem 1.5rem" }}>
      ...
      {invitations.length > 0 && (
        <div style={{ marginBottom: "1.5rem", display: "flex", flexDirection: "column", gap: "0.5rem" }}>
          {invitations.map((inv: { id: string; orgName: string; invitedByName: string; orgSlug: string }) => (
            <div key={inv.id} style={{
              ...
              <span style={{ fontSize: "0.85rem" }}>
                ...
                {inv.invitedByName && <span style={{ color: "var(--text-tertiary)" }}> (by {inv.invitedByName})</span>}
              </span>
      ...
      {myAssignments.length > 0 && (
        <div style={{ marginBottom: "2rem" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
            {myAssignments.map((a: { id: string; courseId: string; repoName: string; ownerName: string; dueDate: string | null; note: string | null; orgName: string }) => {
              const isOverdue = a.dueDate && new Date(a.dueDate) < new Date();
              return (
                <Link key={a.id} href={`/course/${a.courseId}`} style={{ textDecoration: "none", color: "inherit" }}>
                  <div className="card" style={{
                    display: "flex",
                    alignItems: "center",
                    justifyContent: "space-between",
                    padding: "0.75rem 1rem",
                    border: isOverdue ? "1px solid var(--error)" : undefined,
                  }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
                      <span className="badge" style={{ background: "var(--accent-light)", color: "var(--accent)", flexShrink: 0 }}>
                        Assigned
                      </span>
      ...
      {isLoading ? (
        ...
        <div style={{
          ...
          <p style={{ color: "var(--error)", marginBottom: "1rem" }}>Failed to load courses</p>
        ...
        <div style={{
          ...
          {courses.map((course) => {
            const statusStyle = STATUS_COLORS[course.status] || STATUS_COLORS.pending;
            return (
              ...
              onClick={() => {
                if (course.status === "completed") router.push(`/course/${course.id}`);
                else if (course.status === "generating" || course.status === "pending")
              ...
              >
                <div style={{
                  ...
                  {course.version > 1 && course.status === "completed" && course.version > course.lastSeenVersion && (
                    <span className="badge" style={{
                      background: "#E8F5E9",
                      color: "#2E7D32",
                      flexShrink: 0,
                ...
                {course.changesSince && course.version > 1 && course.version > course.lastSeenVersion && (
                  <div style={{
                    background: "#E8F5E9",
                    ...
                    <strong style={{ fontSize: "0.7rem", textTransform: "uppercase", letterSpacing: "0.03em" }}>
                      What changed:
                    </strong>{" "}
                    {course.changesSince.summary}
                    {course.changesSince.changedFiles.length > 0 && (
                      <details style={{ marginTop: "0.375rem", fontSize: "0.75rem" }} onClick={(e) => e.stopPropagation()}>
                        <summary style={{ cursor: "pointer", fontWeight: 600 }}>
                          ℹ️ {course.changesSince.changedFiles.length} files changed since your last view
                        </summary>
                        <div style={{ marginTop: "0.25rem", fontFamily: "var(--font-mono)", fontSize: "0.7rem", maxHeight: 120, overflowY: "auto" }}>
                ...
                <div style={{ display: "flex", gap: "0.375rem", flexWrap: "wrap" }}>
                  ...
                  {course.techStack?.languages?.slice(0, 3).map((l) => (
                    <span key={l} className="badge" style={{ background: "var(--teal-light)", color: "var(--teal)" }}>
                      {l}
                    </span>
                  ))}
                  {course.techStack?.frameworks?.slice(0, 2).map((f) => (
                    <span key={f} className="badge" style={{ background: "#F0F4FF", color: "#4A5BC7" }}>
                      {f}
                    </span>
                  ))}
                  {course.difficulty && (
                    <span className="badge" style={{
                      background: course.difficulty.toLowerCase() === "beginner" ? "var(--teal-light)" :
                                  course.difficulty.toLowerCase() === "advanced" ? "#FFF0EE" : "#FFF8E1",
                      color: course.difficulty.toLowerCase() === "beginner" ? "var(--teal)" :
                             course.difficulty.toLowerCase() === "advanced" ? "var(--accent)" : "var(--warning)",
                    }}>
                      {course.difficulty}
                    </span>
                  )}
                ...
                <div style={{
                  ...
                  >
                    ...
                    {course.status === "completed" && course.createdBy === user?.id && (
                      <button
                        className="btn-ghost"
                        onClick={() => handleWebhookToggle(course.id, !!(course.hasWebhook && course.webhookAutoRegenerate))}
                        title={course.hasWebhook && course.webhookAutoRegenerate ? "Disable auto-updates" : "Enable auto-updates"}
                        style={{ color: course.hasWebhook && course.webhookAutoRegenerate ? "#1565C0" : undefined }}
                      >
                        {course.hasWebhook && course.webhookAutoRegenerate ? "📡" : "📡"}
                      </button>
                    )}
