...
export async function POST(request: Request) {
  ...
  const { githubUrl, targetAudience, organizationSlug, depth, focusAreas, customContext } = body;
  ...
  try {
    ...
    console.error("GitHub URL parse error:", error instanceof Error ? error.message : error);
  ...
  try {
    ...
    if (ghRes.ok) {
      ...
      stars = typeof ghData.stargazers_count === "number" ? ghData.stargazers_count : null;
  ...
  const [course] = await db
    ...
    .values({
      ...
      customContext: typeof customContext === "string" ? customContext.slice(0, 2000) : null,
      status: "pending",
      createdBy: user.id,
      organizationId,
      stars,
